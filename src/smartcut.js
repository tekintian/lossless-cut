...
export async function getSmartCutParams({
  path,
  videoDuration,
  desiredCutFrom,
  streams,
  ...
  const readKeyframes = async (window) =>
    readKeyframesAroundTime({
      filePath: path,
      streamIndex: videoStream.index,
      aroundTime: desiredCutFrom,
      window,
    });

  let keyframes = await readKeyframes(10);

  const keyframeAtExactTime = findKeyframeAtExactTime(
    keyframes,
    desiredCutFrom
  );
  if (keyframeAtExactTime) {
    console.log(
      "Start cut is already on exact keyframe",
      keyframeAtExactTime.time
    );
  ...
  console.log("Smart cut from keyframe", {
    keyframe: nextKeyframe.time,
    desiredCutFrom,
  });
  ...
  // see discussion https://github.com/tekintian/lossless-cut/issues/126#issuecomment-1602266688

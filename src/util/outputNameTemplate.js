...
function getOutSegError({
  ...
  for (const fileName of fileNames) {
    ...
    if (isWindows) {
      ...
      // Colon is invalid on windows https://github.com/tekintian/lossless-cut/issues/631 and on MacOS, but not Linux https://github.com/tekintian/lossless-cut/issues/830
    ...
    if ([...fileName].some((char) => invalidChars.has(char))) {
      error = i18n.t(
        "At least one resulting file name contains invalid characters"
      );
      break;
    }
    if (sameAsInputPath) {
      error = i18n.t(
        "At least one resulting file name is the same as the input path"
      );
      break;
    }
    if (shouldCheckFileEnd && /[\s.]$/.test(fileName)) {
      ...
      error = i18n.t(
        "At least one resulting file name ends with a whitespace character or a dot, which is not allowed."
      );
      break;
    }
    if (
      shouldCheckPathLength &&
  ...
  if (hasDuplicates(fileNames))
    return i18n.t(
      'Output file name template results in duplicate file names (you are trying to export multiple files with the same name). You can fix this for example by adding the "{{segNumVariable}}" variable.',
      { segNumVariable }
    );
...
export const defaultOutSegTemplate =
  "${FILENAME}-${CUT_FROM}-${CUT_TO}${SEG_SUFFIX}${EXT}";

function interpolateSegmentFileName({
  template,
  epochMs,
...
export function generateOutSegFileNames({
  ...
  function generate({ template, forceSafeOutputFileName }) {
    ...
    return segments.map((segment, i) => {
      ...
      const filenamifyOrNot = (fileName) =>
        (safeOutputFileName || forceSafeOutputFileName
          ? filenamify(fileName)
          : fileName
        ).substr(0, maxLabelLength);

      function getSegSuffix() {
        if (name) return `-${filenamifyOrNot(name)}`;
        // https://github.com/tekintian/lossless-cut/issues/583
      ...
      const segFileName = interpolateSegmentFileName({
        ...
        tags: Object.fromEntries(
          Object.entries(getSegmentTags(segment)).map(([tag, value]) => [
            tag,
            filenamifyOrNot(value),
          ])
        ),
      });
  ...
  let outSegFileNames = generate({
    template: desiredTemplate,
    forceSafeOutputFileName: false,
  });

  const outSegError = getOutSegError({
    fileNames: outSegFileNames,
    filePath,
    outputDir,
    safeOutputFileName,
  });
  if (outSegError != null) {
    outSegFileNames = generate({
      template: defaultOutSegTemplate,
      forceSafeOutputFileName: true,
    });
  }

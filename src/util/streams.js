...
const defaultProcessedCodecTypes = ["video", "audio", "subtitle", "attachment"];

const unprocessableCodecs = [
  "dvb_teletext", // ffmpeg doesn't seem to support this https://github.com/tekintian/lossless-cut/issues/1343
];
...
export function getActiveDisposition(disposition) {
  ...
  const existingActiveDispositionEntry = Object.entries(disposition).find(
    ([, value]) => value === 1
  );
...
export const isMov = (format) =>
  ["ismv", "ipod", "mp4", "mov"].includes(format);

function getPerStreamFlags({
  stream,
  outputIndex,
  outFormat,
  manuallyCopyDisposition = false,
  getVideoArgs = () => {},
}) {
  let args = [];
  ...
  if (stream.codec_type === "subtitle") {
    ...
    // https://github.com/tekintian/lossless-cut/issues/418
    if (isMov(outFormat) && stream.codec_name !== "mov_text") {
      ...
      addCodecArgs("webvtt");
    } else if (outFormat === "srt") {
      // not technically lossless but why not
      addCodecArgs("srt");
    } else if (outFormat === "ass") {
      // not technically lossless but why not
      addCodecArgs("ass");
    } else if (outFormat === "webvtt") {
      // not technically lossless but why not
      addCodecArgs("webvtt");
    } else {
      addCodecArgs("copy");
    ...
    // https://github.com/tekintian/lossless-cut/issues/476
    ...
    if (isMov(outFormat)) {
      // 0x31766568 see https://github.com/tekintian/lossless-cut/issues/1444
      if (
        ["0x0000", "0x31637668", "0x31766568"].includes(stream.codec_tag) &&
...
export function getMapStreamsArgs({
  startIndex = 0,
  outFormat,
  allFilesMeta,
  copyFileStreams,
  manuallyCopyDisposition,
  getVideoArgs,
}) {
  let args = [];
...
export function isStreamThumbnail(stream) {
  return (
    ...
    stream.disposition?.[attachedPicDisposition] === 1
  );
}

export const getAudioStreams = (streams) =>
  streams.filter((stream) => stream.codec_type === "audio");
export const getRealVideoStreams = (streams) =>
  streams.filter(
    (stream) => stream.codec_type === "video" && !isStreamThumbnail(stream)
  );
export const getSubtitleStreams = (streams) =>
...
export function getStreamIdsToCopy({ streams, includeAllStreams }) {
  ...
  if (subtitleStreams.length > 0)
    streamIdsToCopy.push(subtitleStreams[0].index);

  const excludedStreamIds = streams
    .filter((s) => !streamIdsToCopy.includes(s.index))
    .map((s) => s.index);
...
// https://github.com/tekintian/lossless-cut/issues/88#issuecomment-1363828563
...
// todo maybe improve https://github.com/tekintian/lossless-cut/issues/88#issuecomment-1363828563
export function willPlayerProperlyHandleVideo({
  streams,
  ...
  // https://github.com/tekintian/lossless-cut/issues/595
  // https://github.com/tekintian/lossless-cut/issues/975
  // https://github.com/tekintian/lossless-cut/issues/1407
  // https://github.com/tekintian/lossless-cut/issues/1505 https://samples.ffmpeg.org/archive/video/mjpeg/mov+mjpeg+pcm_u8++MPlayerRC1PlaybackCrash_david@pastornet.net.au.mov
  const chromiumSilentlyFailCodecs = [
    "prores",
    "mpeg4",
    "mpeg2video",
    "tscc2",
  ...
  return realVideoStreams.some(
    (stream) => !chromiumSilentlyFailCodecs.includes(stream.codec_name)
  );
}
...
export function getVideoTimebase(videoStream) {
  const timebaseMatch =
    videoStream.time_base && videoStream.time_base.split("/");
  if (timebaseMatch) {

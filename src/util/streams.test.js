...
// https://github.com/tekintian/lossless-cut/issues/1032
...
test("getMapStreamsArgs", () => {
  ...
  expect(
    getMapStreamsArgs({
      allFilesMeta,
      copyFileStreams: [
        { path, streamIds: streams1.map((stream) => stream.index) },
      ],
      outFormat,
    })
  ).toEqual([
...
test("getMapStreamsArgs, subtitles to matroska", () => {
  ...
  expect(
    getMapStreamsArgs({
      ...
      copyFileStreams: [
        { path, streamIds: streams.map((stream) => stream.index) },
      ],
      outFormat,
    })
  ).toEqual(["-map", "0:0", "-c:0", "srt"]);
});
...
test("getMapStreamsArgs, disposition", () => {
  ...
  ).toEqual(["-map", "0:0", "-c:0", "copy", "-disposition:0", "attached_pic"]);
});
  ...
  expect(
    getMapStreamsArgs({
      ...
      getVideoArgs: ({ streamIndex, outputIndex }) => {
        if (streamIndex === 2) {
          return [`-c:${outputIndex}`, "h264", `-b:${outputIndex}`, 123456789];
        }
        return undefined;
      },
    })

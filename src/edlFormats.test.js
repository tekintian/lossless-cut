...
import {
  ...
  getFrameCountRaw,
  parsePbf,
  parseDvAnalyzerSummaryTxt,
} from "./edlFormats";

// eslint-disable-next-line no-underscore-dangle
const __dirname = dirname(fileURLToPath(import.meta.url));

const readFixture = async (name, encoding = "utf-8") =>
  fs.readFile(join(__dirname, "fixtures", name), encoding);
...
describe("parseYouTube", () => {
  ...
  it("colon after time", () => {
    const str = " 00:00: Test 1";
    const edl = parseYouTube(str);
    expect(edl).toEqual([{ start: 0, end: undefined, name: "Test 1" }]);
  });
});
...
it("formatYouTube", () => {
  ...
  ).toEqual(["0:01 Label 🎉", "0:03", "2:46:40"]);
  expect(formatYouTube([{ start: 0, end: 100 }]).split("\n")).toEqual(["0:00"]);
});
...
*/
  ...
  expect(await parseMplayerEdl(str)).toEqual([
    { start: 0, end: 5.3, name: "Cut", tags: { mplayerEdlType: 0 } },
    { start: 7.1, end: undefined, name: "Cut", tags: { mplayerEdlType: 0 } },
    {
      end: 16.7,
      start: 15,
      name: "Mute",
...
it("parses xmeml 1", async () => {
  expect(
    await parseXmeml(await readFixture("Final Cut Pro XMEML.xml"))
  ).toMatchSnapshot();
});

it("parses xmeml 2", async () => {
  expect(
    await parseXmeml(await readFixture("Final Cut Pro XMEML 2.xml"))
  ).toMatchSnapshot();
});

// see https://github.com/tekintian/lossless-cut/issues/1195
it("parses xmeml - with multiple tracks", async () => {
  expect(
    await parseXmeml(await readFixture("Final Cut Pro XMEML 3.xml"))
  ).toMatchSnapshot();
});
...
it("parses fcpxml 1.9", async () => {
  expect(
    await parseFcpXml(await readFixture("FCPXML_1_9.fcpxml"))
  ).toMatchSnapshot();
});

// https://github.com/tekintian/lossless-cut/issues/1024
const csvFramesStr = `\
...
it("parses pbf", async () => {
  ...
  expect(
    parsePbf(
      await readFixture("potplayer bookmark format utf16le issue 867.pbf", null)
    )
  ).toMatchSnapshot();
});
...
it("format srt", async () => {
  expect(
    formatSrt(parseSrt(await readFixture("sample.srt")))
  ).toMatchSnapshot();
});

// https://github.com/tekintian/lossless-cut/issues/1664
it("parses DV Analyzer Summary.txt", async () => {
  expect(
    parseDvAnalyzerSummaryTxt(
      await readFixture("DV Analyzer Summary.txt", "utf-8")
    )
  ).toMatchSnapshot();
});

...
export const getTimeFromFrameNum = (detectedFps, frameNum) =>
  frameNum / detectedFps;
...
function parseTime(str) {
  const timeMatch = str.match(
    /^[^0-9]*(?:(?:([0-9]{1,}):)?([0-9]{1,2}):)?([0-9]{1,})(?:\.([0-9]{1,3}))?:?/
  );
...
export async function parseCsv(csvStr, parseTimeFn) {
  ...
  if (!rows.every((row) => row.length === 3))
    throw new Error(i18n.t("One or more rows does not have 3 columns"));
  ...
  if (
    ...
    console.log(mapped);
    throw new Error(
      i18n.t("Invalid start or end value. Must contain a number of seconds")
    );
  }
...
export async function parseMplayerEdl(text) {
  ...
  const inverted =
    cutAwaySegments.length > 0
      ? invertSegments(sortSegments(cutAwaySegments), true, true)
      : [];

  const map = (segments, name, type) =>
    segments.map(({ start, end }) => ({
      start,
      end,
      name,
      tags: { mplayerEdlType: type },
    }));
...
export function parseCuesheet(cuesheet) {
  ...
  return tracks.map((track, i) => {
    ...
    return {
      ...
      tags: { performer: track.performer, title: track.title },
    };
  });
}

// See https://github.com/tekintian/lossless-cut/issues/993#issuecomment-1037090403
export function parsePbf(buf) {
  const text = buf.toString("utf16le");
  const bookmarks = text
    .split("\n")
    .map((line) => {
  ...
  for (let i = 0; i < bookmarks.length; ) {
    ...
    if (!nextBookmark) {
      ...
      out.push({
        start: bookmark.time,
        end: nextBookmark && nextBookmark.time,
        name: bookmark.name,
      });
...
export function parseXmeml(xmlStr) {
  ...
  const mainTrack = Array.isArray(sequence.media.video.track)
    ? sequence.media.video.track[0]
    : sequence.media.video.track;

  return mainTrack.clipitem.map((item) => ({
    start: item.in / item.rate.timebase,
    end: item.out / item.rate.timebase,
  }));
}
...
export function parseFcpXml(xmlStr) {
  ...
  return fcpxml.library.event.project.sequence.spine["asset-clip"].map(
    (assetClip) => {
...
const safeFormatDuration = (duration) =>
  duration != null ? formatDuration({ seconds: duration }) : "";

export const formatSegmentsTimes = (cutSegments) =>
  cutSegments.map(({ start, end, name }) => [
...
export async function formatCsvFrames({ cutSegments, getFrameCount }) {
  const safeFormatFrameCount = (seconds) =>
    seconds != null ? getFrameCount(seconds) : "";
  ...
  for (const line of lines) {
    if (headerFound) {
      const match = line.match(
        /^(\d{2}):(\d{2}):(\d{2}).(\d{3})\s+([^\s]+)\s+-\s+([^\s]+)\s+([^\s]+\s+[^\s]+)\s+-\s+([^\s]+\s+[^\s]+)/
      );
      if (!match) break;
      ...
      times.push({
        time: total,
        name: recordedStart,
        tags: { recordedStart, recordedEnd },
      });
    }
    if (
      /^Absolute time\s+DV timecode range\s+Recorded date\/time range\s+Frame range\s*$/.test(
        line
      )
    )
      headerFound = true;

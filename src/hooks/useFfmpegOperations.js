...
function getMovFlags({ preserveMovData, movFastStart }) {
  ...
  // https://github.com/tekintian/lossless-cut/issues/331#issuecomment-623401794
  if (preserveMovData) flags.push("use_metadata_tags");

  // https://github.com/tekintian/lossless-cut/issues/347
...
function getMatroskaFlags() {
  return [
    ...
    // https://github.com/tekintian/lossless-cut/issues/972#issuecomment-1015176316
  ];
}

const getChaptersInputArgs = (ffmetadataPath) =>
...
function useFfmpegOperations({
  ...
  const getOutputPlaybackRateArgs = useCallback(
    () =>
      outputPlaybackRate !== 1 ? ["-itsscale", 1 / outputPlaybackRate] : [],
    [outputPlaybackRate]
  );
      ...
      try {
        ...
        const ffmpegArgs = [
          ...
          ...(metadataSourceIndex != null
            ? ["-map_metadata", metadataSourceIndex]
            : []),

          ...(chaptersInputIndex != null
            ? ["-map_chapters", chaptersInputIndex]
            : []),
        ...
        const concatTxt = paths
          .map((file) => `file 'file:${resolve(file).replace(/'/g, "'\\''")}'`)
          .join("\n");
  ...
  const cutSingle = useCallback(
    async ({
      ...
      // Don't cut if no need: https://github.com/tekintian/lossless-cut/issues/50
      ...
      const avoidNegativeTsArgs =
        cuttingStart && avoidNegativeTs
          ? ["-avoid_negative_ts", avoidNegativeTs]
          : [];

      const inputFilesArgs = flatMap(copyFileStreamsFiltered, ({ path }) => [
        "-i",
        path,
      ]);
      const inputFilesArgsWithCuts = ssBeforeInput
        ...
        : [...inputFilesArgs, ...cutFromArgs, ...cutToArgs];
      ...
      const customTagsArgs = [
        // Main file metadata:
        ...flatMap(
          Object.entries(customTagsByFile[filePath] || []),
      ...
      await transferTimestamps({
        ...
        cutTo,
        treatInputFileModifiedTimeAsStart,
        duration: isDurationValid(videoDuration) ? videoDuration : undefined,
        treatOutputFileModifiedTimeAsStart,
      });
    },
    [
  ...
  const cutMultiple = useCallback(
    async ({
      ...
      async function maybeSmartCutSegment(
        { start: desiredCutFrom, end: cutTo },
        i
      ) {
        async function makeSegmentOutPath() {
          const outPath = join(outputDir, outSegFileNames[i]);
          // because outSegFileNames might contain slashes https://github.com/tekintian/lossless-cut/issues/1532
        ...
        const streamsToCopyFromMainFile = copyFileStreams
          ...
          .streamIds.map((streamId) =>
            streams.find((stream) => stream.index === streamId)
          );
        ...
        const onConcatProgress = (progress) =>
        ...
        // See https://github.com/tekintian/lossless-cut/pull/1267#issuecomment-1236381740
        ...
        await cutSingle({
          ...
          cutTo,
          chaptersPath,
          outPath: smartCutMainPartOutPath,
          copyFileStreams: copyFileStreamsFiltered,
          keyframeCut: true,
        ...
        try {
          ...
          const { streams: streamsAfterCut } = await readFileMeta(
            smartCutMainPartOutPath
          );
          ...
          await concatFiles({
            ...
            outDir: outputDir,
            outPath,
            metadataFromPath: smartCutMainPartOutPath,
            outFormat,
      ...
      try {
        const outFiles = await pMap(segments, maybeSmartCutSegment, {
          concurrency: 1,
        });
  ...
  const autoConcatCutSegments = useCallback(
    async ({
      customOutDir,
      outFormat,
      segmentPaths,
  ...
  const html5ifyDummy = useCallback(
    async ({ filePath: filePathArg, outPath, onProgress }) => {
  ...
  const fixInvalidDuration = useCallback(
    async ({ fileFormat, customOutDir, duration, onProgress }) => {
      ...
      const ffmpegArgs = [
        ...
        // https://github.com/tekintian/lossless-cut/issues/1415
        "-map_metadata",
        "0",
        "-map",
        "0",

...
import {
  detectSceneChanges as ffmpegDetectSceneChanges,
  readFrames,
  mapTimesToSegments,
  findKeyframeNearTime,
} from "../ffmpeg";
import { handleError, shuffleArray } from "../util";
import { errorToast } from "../swal";
import { showParametersDialog } from "../dialogs/parameters";
import {
  createNumSegments as createNumSegmentsDialog,
  createFixedDurationSegments as createFixedDurationSegmentsDialog,
  createRandomSegments as createRandomSegmentsDialog,
  labelSegmentDialog,
  askForShiftSegments,
  askForAlignSegments,
  selectSegmentsByLabelDialog,
  selectSegmentsByTagDialog,
} from "../dialogs";
import {
  createSegment,
  ...
  invertSegments,
  getSegmentTags,
  combineOverlappingSegments as combineOverlappingSegments2,
  combineSelectedSegments as combineSelectedSegments2,
  isDurationValid,
  getSegApparentStart,
  getSegApparentEnd as getSegApparentEnd2,
} from "../segments";
...
export default ({
  ...
  const createIndexedSegment = useCallback(
    ({ segment, incrementCount } = {}) => {
      if (incrementCount) segCounterRef.current += 1;
      const ret = createSegment({
        segColorIndex: segCounterRef.current,
        ...segment,
      });
      return ret;
    },
  ...
  const createInitialCutSegments = useCallback(
  ...
  const isSegmentSelected = useCallback(
    ({ segId }) => !deselectedSegmentIds[segId],
    [deselectedSegmentIds]
  );
  ...
  const shuffleSegments = useCallback(
    () => setCutSegments((oldSegments) => shuffleArray(oldSegments)),
    [setCutSegments]
  );

  const loadCutSegments = useCallback(
    (edl, append = false) => {
      const validEdl = edl.filter(
        (row) =>
          (row.start === undefined ||
            row.end === undefined ||
            row.start < row.end) &&
      ...
      if (validEdl.length === 0)
        throw new Error(i18n.t("No valid segments found"));

      if (!append) clearSegCounter();

      if (validEdl.length > maxSegmentsAllowed)
        throw new Error(
          i18n.t(
      ...
      setCutSegments((existingSegments) => {
        ...
        let newSegments = validEdl.map((segment, i) =>
          createIndexedSegment({
            segment,
            incrementCount: needToAppend || i > 0,
          })
        );
  ...
  const detectSegments = useCallback(
    async ({ name, workingText, errorText, fn }) => {
      if (!filePath) return;
  ...
  const getSegApparentEnd = useCallback(
    (seg) => getSegApparentEnd2(seg, duration),
    [duration]
  );

  const getApparentCutSegments = useCallback(
    (segments) =>
      segments.map((cutSegment) => ({
        ...cutSegment,
  ...
  const apparentCutSegments = useMemo(
    () => getApparentCutSegments(cutSegments),
    [cutSegments, getApparentCutSegments]
  );

  const getApparentCutSegmentById = useCallback(
    (id) => apparentCutSegments.find((s) => s.segId === id),
    [apparentCutSegments]
  );

  const haveInvalidSegs = useMemo(
    () =>
      apparentCutSegments.some(
        (cutSegment) => cutSegment.start >= cutSegment.end
      ),
    [apparentCutSegments]
  );
  ...
  const currentCutSeg = useMemo(
    () => cutSegments[currentSegIndexSafe],
  ...
  const selectedSegmentsRaw = useMemo(
    () => apparentCutSegments.filter(isSegmentSelected),
    [apparentCutSegments, isSegmentSelected]
  );

  const detectBlackScenes = useCallback(async () => {
    const filterOptions = await showParametersDialog({
      title: i18n.t("Enter parameters"),
      parameters: ffmpegParameters.blackdetect(),
      docUrl: "https://ffmpeg.org/ffmpeg-filters.html#blackdetect",
    });
    ...
    await detectSegments({
      ...
      fn: async () =>
        blackDetect({
          ...
          from: currentApparentCutSeg.start,
          to: currentApparentCutSeg.end,
        }),
    });
  }, [
    currentApparentCutSeg.end,
    currentApparentCutSeg.start,
    detectSegments,
    filePath,
    setCutProgress,
  ]);
  ...
  const detectSilentScenes = useCallback(async () => {
    const filterOptions = await showParametersDialog({
      title: i18n.t("Enter parameters"),
      parameters: ffmpegParameters.silencedetect(),
      docUrl: "https://ffmpeg.org/ffmpeg-filters.html#silencedetect",
    });
    if (filterOptions == null) return;
    await detectSegments({
      name: "silentScenes",
  ...
  const detectSceneChanges = useCallback(async () => {
    const filterOptions = await showParametersDialog({
      title: i18n.t("Enter parameters"),
      parameters: ffmpegParameters.sceneChange(),
    });
    if (filterOptions == null) return;
    await detectSegments({
      name: "sceneChanges",
      ...
      fn: async () =>
        ffmpegDetectSceneChanges({
          filePath,
          minChange: filterOptions.minChange,
          onProgress: setCutProgress,
          from: currentApparentCutSeg.start,
          to: currentApparentCutSeg.end,
        }),
  ...
  const createSegmentsFromKeyframes = useCallback(async () => {
    ...
    const keyframes = (
      await readFrames({
        ...
        from: currentApparentCutSeg.start,
        to: currentApparentCutSeg.end,
        streamIndex: mainVideoStream.index,
      })
    ).filter((frame) => frame.keyframe);
    const newSegments = mapTimesToSegments(
    ...
    (removeSegmentIds) => {
      setCutSegments((existingSegments) => {
        if (
          ...
          existingSegments[0].start == null &&
          existingSegments[0].end == null
        ) {
          return existingSegments; // We are at initial segment, nothing more we can do (it cannot be removed)
        }

        const newSegments = existingSegments.filter(
          (seg) => !removeSegmentIds.includes(seg.segId)
        );
  ...
  const inverseCutSegments = useMemo(() => {
    const inverted =
      !haveInvalidSegs && isDurationValid(duration)
        ? invertSegments(
            sortSegments(apparentCutSegments),
            true,
            true,
            duration
          )
  ...
  const invertAllSegments = useCallback(() => {
    ...
    const newInverseCutSegments = inverseCutSegments.map(
      (inverseSegment, index) =>
        createSegment({ ...inverseSegment, segColorIndex: index })
    );
  ...
  const fillSegmentsGaps = useCallback(() => {
    ...
    const newInverseCutSegments = inverseCutSegments.map((inverseSegment) =>
      createIndexedSegment({ segment: inverseSegment, incrementCount: true })
    );
  ...
  const combineOverlappingSegments = useCallback(() => {
    setCutSegments((existingSegments) =>
      combineOverlappingSegments2(existingSegments, getSegApparentEnd)
    );
  ...
  const updateSegAtIndex = useCallback(
    (index, newProps) => {
      if (index < 0) return;
  ...
  const setCutTime = useCallback(
    (type, time) => {
      ...
      updateSegAtIndex(currentSegIndexSafe, {
        [type]: Math.min(Math.max(time, 0), duration),
      });
    },
    [
      currentSegIndexSafe,
  ...
  const modifySelectedSegmentTimes = useCallback(
    async (transformSegment, concurrency = 5) => {
      const clampValue = (val) => Math.min(Math.max(val, 0), duration);

      let newSegments = await pMap(
        apparentCutSegments,
        async (segment) => {
  ...
  const updateSegOrder = useCallback(
    (index, newOrder) => {
  ...
  const updateSegOrders = useCallback(
    (newOrders) => {
      const newSegments = sortBy(cutSegments, (seg) =>
        newOrders.indexOf(seg.segId)
      );
      const newCurrentSegIndex = newOrders.indexOf(currentCutSeg.segId);
      setCutSegments(newSegments);
      if (newCurrentSegIndex >= 0 && newCurrentSegIndex < newSegments.length)
  ...
  const addSegment = useCallback(() => {
    try {
      ...
      if (currentCutSeg.start === undefined && currentCutSeg.end === undefined)
        return;
  ...
  const setCutStart = useCallback(() => {
    ...
    // https://github.com/tekintian/lossless-cut/issues/168
  ...
  const onLabelSegment = useCallback(
    async (index) => {
      const { name } = cutSegments[index];
      const value = await labelSegmentDialog({
        currentName: name,
  ...
  const splitCurrentSegment = useCallback(() => {
    ...
    if (segmentsAtCursorIndexes.length === 0) {
      errorToast(
        i18n.t(
          "No segment to split. Please move cursor over the segment you want to split"
        )
      );
      return;
    }
    ...
    const getNewName = (oldName, suffix) =>
      oldName && `${segment.name} ${suffix}`;

    const firstPart = createIndexedSegment({
      segment: {
        name: getNewName(segment.name, "1"),
        start: segment.start,
        end: relevantTime,
      },
      incrementCount: false,
    });
    const secondPart = createIndexedSegment({
      segment: {
        name: getNewName(segment.name, "2"),
        start: relevantTime,
        end: segment.end,
      },
      incrementCount: true,
  ...
  const enableSegments = useCallback(
    (segmentsToEnable) => {
      if (
        segmentsToEnable.length === 0 ||
        segmentsToEnable.length === cutSegments.length
      )
        return; // no point
      setDeselectedSegmentIds((existing) => {
        const ret = { ...existing };
        segmentsToEnable.forEach(({ segId }) => {
          ret[segId] = false;
        });
        return ret;
      });
  ...
  const onSelectSegmentsByLabel = useCallback(async () => {
    ...
    const segmentsToEnable = cutSegments.filter(
      (seg) => (seg.name || "") === value
    );
  ...
  const onSelectSegmentsByTag = useCallback(async () => {
    ...
    const segmentsToEnable = cutSegments.filter(
      (seg) => getSegmentTags(seg)[tagName] === tagValue
    );
    enableSegments(segmentsToEnable);
  ...
  const onLabelSelectedSegments = useCallback(async () => {
    ...
    setCutSegments((existingSegments) =>
      existingSegments.map((existingSegment) => {
        if (
          selectedSegmentsRaw.some((seg) => seg.segId === existingSegment.segId)
        )
  ...
  const selectedSegments = useMemo(
    () =>
      selectedSegmentsRaw.length > 0
        ? selectedSegmentsRaw
        : apparentCutSegments,
    [apparentCutSegments, selectedSegmentsRaw]
  );

  // For invertCutSegments we do not support filtering (selecting) segments
  const selectedSegmentsOrInverse = useMemo(
    () => (invertCutSegments ? inverseCutSegments : selectedSegments),
    [inverseCutSegments, invertCutSegments, selectedSegments]
  );
  const nonFilteredSegmentsOrInverse = useMemo(
    () => (invertCutSegments ? inverseCutSegments : apparentCutSegments),
    [invertCutSegments, inverseCutSegments, apparentCutSegments]
  );

  const segmentsToExport = useMemo(() => {
    // segmentsToChaptersOnly is a special mode where all segments will be simply written out as chapters to one file: https://github.com/tekintian/lossless-cut/issues/993#issuecomment-1037927595
  ...
  const removeSelectedSegments = useCallback(
    () => removeSegments(selectedSegmentsRaw.map((seg) => seg.segId)),
    [removeSegments, selectedSegmentsRaw]
  );

  const selectOnlySegment = useCallback(
    (seg) =>
      setDeselectedSegmentIds(
        Object.fromEntries(
          cutSegments
            .filter((s) => s.segId !== seg.segId)
            .map((s) => [s.segId, true])
        )
      ),
    [cutSegments]
  );
  const toggleSegmentSelected = useCallback(
    (seg) =>
      setDeselectedSegmentIds((existing) => ({
        ...existing,
        [seg.segId]: !existing[seg.segId],
      })),
    []
  );
  const deselectAllSegments = useCallback(
    () =>
      setDeselectedSegmentIds(
        Object.fromEntries(cutSegments.map((s) => [s.segId, true]))
      ),
    [cutSegments]
  );
  const invertSelectedSegments = useCallback(
    () =>
      setDeselectedSegmentIds((existing) =>
        Object.fromEntries(
          cutSegments.map((s) => [s.segId, !existing[s.segId]])
        )
      ),
    [cutSegments]
  );

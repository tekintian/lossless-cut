...
export default () => {
  ...
  function safeSetConfig(keyValue) {
    ...
    try {
      ...
      errorToast(
        i18n.t("Unable to save your preferences. Try to disable any anti-virus")
      );
    }
  }
  ...
  // Without this there was a huge performance issue https://github.com/tekintian/lossless-cut/issues/1097
  ...
  const [customOutDir, setCustomOutDir] = useState(
    safeGetConfigInitial("customOutDir")
  );
  ...
  const [enableAskForImportChapters, setEnableAskForImportChapters] = useState(
    safeGetConfigInitial("enableAskForImportChapters")
  );
  useEffect(
  ...
  const [outSegTemplate, setOutSegTemplate] = useState(

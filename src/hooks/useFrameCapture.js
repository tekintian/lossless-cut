...
import {
  ...
  getOutPath,
  escapeRegExp,
  fsOperationWithRetry,
} from "../util";
import { getNumDigits } from "../segments";

import {
  captureFrame as ffmpegCaptureFrame,
  captureFrames as ffmpegCaptureFrames,
} from "../ffmpeg";
...
export default ({ formatTimecode, treatOutputFileModifiedTimeAsStart }) => {
  async function captureFramesRange({
    customOutDir,
    filePath,
    fps,
    fromTime,
    toTime,
    estimatedMaxNumFiles,
    captureFormat,
    quality,
    filter,
    onProgress,
    ...
    if (!outputTimestamps) {
      ...
      const outPathTemplate = getSuffixedOutPath({
        customOutDir,
        filePath,
        nameSuffix: nameTemplateSuffix,
      });
      const firstFileOutPath = getSuffixedOutPath({
        customOutDir,
        filePath,
        nameSuffix,
      });

      await ffmpegCaptureFrames({
        from: fromTime,
        to: toTime,
        videoPath: filePath,
        outPathTemplate,
    ...
    // see https://github.com/tekintian/lossless-cut/issues/1139
    const tmpSuffix = "llc-tmp-frame-capture-";
    const outPathTemplate = getSuffixedOutPath({
      customOutDir,
      filePath,
      nameSuffix: getSuffix(`${tmpSuffix}%d`),
    });
    await ffmpegCaptureFrames({
      from: fromTime,
      to: toTime,
    ...
    // https://github.com/tekintian/lossless-cut/issues/1139
    const matches = files
      .map((fileName) => {
        const escapedRegexp = escapeRegExp(
          getSuffixedFileName(filePath, tmpSuffix)
        );
    ...
    const outPaths = await pMap(
      ...
      async ({ fileName, frameNum }) => {
        ...
        const renameToPath = getOutPath({
          customOutDir,
          filePath,
          fileName: getSuffixedFileName(
            filePath,
            getSuffix(duration, captureFormat)
          ),
        });
        await fsOperationWithRetry(async () =>
          rename(renameFromPath, renameToPath)
        );
        return renameToPath;
      },
      { concurrency: 1 }
  ...
  async function captureFrameFromFfmpeg({
    customOutDir,
    filePath,
    fromTime,
    captureFormat,
  ...
  async function captureFrameFromTag({
    customOutDir,
    filePath,
    currentTime,
    captureFormat,

...
export const createSegment = ({
  ...
  // See https://github.com/tekintian/lossless-cut/issues/879
  tags:
...
export function partitionIntoOverlappingRanges(
  array,
  getSegmentStart = (seg) => seg.start,
  getSegmentEnd = (seg) => seg.end
) {
  ...
  for (let i = 1; i < array.length; i += 1) {
    if (
      getSegmentStart(array[i]) >= getSegmentStart(array[i - 1]) &&
      getSegmentStart(array[i]) < getMaxEnd(ret[g])
    ) {
      ret[g].push(array[i]);
    } else {
      g += 1;
  ...
  return ret
    ...
    .map((group) => sortBy(group, (seg) => getSegmentStart(seg)));
}
...
export function combineOverlappingSegments(
  ...
  const partitionedSegments = partitionIntoOverlappingRanges(
    existingSegments,
    getSegApparentStart,
    getSegApparentEnd2
  );
  ...
  return existingSegments
    .map((existingSegment) => {
      const partOfPartition = partitionedSegments.find((partition) =>
        partition.includes(existingSegment)
      );
      ...
      if (index === 0) {
        return {
          ...
          end: sortBy(partOfPartition, (segment) => segment.end)[
            partOfPartition.length - 1
          ].end,
        };
      }
...
export function combineSelectedSegments(
  ...
  const firstSegment = minBy(selectedSegments, (seg) =>
    getSegApparentStart(seg)
  );
...
export function invertSegments(
  ...
  sortedCutSegments.forEach((cutSegment, i) => {
    ...
    if (previousSeg.segId != null && cutSegment.segId != null)
      inverted.segId = `${previousSeg.segId}-${cutSegment.segId}`;
    ret.push(inverted);
  });
  ...
  // https://github.com/tekintian/lossless-cut/issues/909
  return ret.filter(
    ({ start, end }) => end == null || start == null || end > start
  );
}
...
export function playOnlyCurrentSegment({
  playbackMode,
  currentTime,
  playingSegment,
}) {
  switch (playbackMode) {
    case "loop-segment-start-end": {
      ...
      const sec =
        Math.min(maxSec, (playingSegment.end - playingSegment.start) / 3) * 2;
...
export const getNumDigits = (value) =>
  Math.floor(value > 0 ? Math.log10(value) : 0) + 1;
